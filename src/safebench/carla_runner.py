...
class CarlaRunner:
    def __init__(self, agent_config, scenario_config):
        ...
        self.env_params = {
            ...
            'd_behind': 12,                                            # distance wbehind the ego vehicle (meter)
        ...
        if self.mode == 'train_agent':
            self.buffer_capacity = agent_config['buffer_capacity'] # buffer of RL
    ...
    def train(self, data_loader, start_episode=0):
        ...
        for e_i in tqdm(range(start_episode, self.train_episode)):
            ...
            static_obs = self.env.get_static_obs(sampled_scenario_configs) # AV的路线和目标速度观测值
    ...
    def eval(self, data_loader):
        ...
        while len(data_loader) > 0:
            ...
            while not self.env.all_scenario_done():
                ...
                # apply action to env and get obs # 所有学习算法的reward是相同的！
    ...
    def run(self):
        ...
        for m_i in config_by_map.keys():
            ...
            self._init_renderer() # gym渲染，此处可能线程报错，多试一试即可
            ...
            if self.mode == 'eval':
                self.agent_policy.load_model() # agent_policy发生变化，自动找到basic、ppo、ddpg等模型加载
                ...
                # start_episode = self.check_continue_training(self.scenario_policy) #caixuan
                start_episode = 0
